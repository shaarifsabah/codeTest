...
class BookingRepository extends BaseRepository
{
    ...
    {
        $cUser = $this->_userRepo->find($user_id);
        $usertype = '';
        $emergencyJobs = [];
        $normalJobs = [];
        if ($cUser && $cUser->is(IUserTypes::CUSTOMER)) {
            $jobs = $cUser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback')
                    ->whereIn('status', ['pending', 'assigned', 'started'])
                    ->orderBy('due', 'asc')
                    ->get();
            $usertype = IUserTypes::CUSTOMER;
        } elseif ($cUser && $cUser->is(IUserTypes::TRANSLATOR)) {
            $jobs = Job::getTranslatorJobs($cUser->id, 'new');
            $jobs = $jobs->pluck('jobs')->all();
            $usertype = IUserTypes::TRANSLATOR;
        }
        if ($jobs) {
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == IUserRoles::CUSTOMER_ROLE) {
            $cuser = $user;
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        foreach ($users as $oneUser) {
            if ($oneUser->user_type == IUserTypes::TRANSLATOR && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
